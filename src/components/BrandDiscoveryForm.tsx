...
import { 
  ...
  BookOpen, Layout, Database, Download, X
} from 'lucide-react';
...
export const BrandDiscoveryForm = ({ initialData, onUpdate, onComplete, addNotification }: Props) => {
  ...
  const handleManualSave = useCallback(async () => {
    ...
    window.dispatchEvent(new CustomEvent('brandforge:discovery-save-status', { detail: 'saving' }));
    try {
      if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
      await onUpdate(data);
      addNotification({
      ...
      setTimeout(() => {
        setSaveStatus('saved');
        window.dispatchEvent(new CustomEvent('brandforge:discovery-save-status', { detail: 'saved' }));
        setTimeout(() => {
          setSaveStatus('idle');
          window.dispatchEvent(new CustomEvent('brandforge:discovery-save-status', { detail: 'idle' }));
        }, 2000);
      }, 500);
    } catch (err) {
      addNotification({
        title: 'Sync Failed',
        type: 'error',
        message: 'Could not update project data.'
      });
      setSaveStatus('idle');
      window.dispatchEvent(new CustomEvent('brandforge:discovery-save-status', { detail: 'idle' }));
    }
  }, [data, onUpdate, addNotification]);

  const handleExportPDF = useCallback(async () => {
    await addNotification({
      title: 'Generating Blueprint',
      type: 'info',
      message: 'Constructing your professional Discovery document...'
    });
    ...
    try {
      ...
      // --- HELPERS (Synced with Strategy Tool Pattern) ---
      const addHeaderFooter = () => {
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(8);
        pdf.setTextColor(113, 113, 122); // brand-500
        pdf.text('BRAND DISCOVERY BLUEPRINT', margin, margin - 10);
        ...
        pdf.text('Generated by BrandForge Intelligence Suite', margin, pageHeight - 25);
      ...
      pdf.text(data.name || data.registeredName || 'Brand Project', pageWidth / 2, y, { align: 'center' });
      
      y += 30;
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(16);
      pdf.setTextColor(113, 113, 122); // brand-500
      pdf.text('Brand Discovery Blueprint', pageWidth / 2, y, { align: 'center' });
      ...
      addTitle('01. Core Identity');
      addField('Full Name', data.fullName);
      addField('Title', data.ownerTitle);
      addField('Registered Name', data.registeredName);
      addField('Tagline', data.tagline);
      addField('Industry', data.industryOther || data.industry);
      addField('Stage', data.stageOther || data.stage);
      
      addDivider();
      addTitle('02. Strategic Pillars');
      addField('Mission', data.mission);
      addField('Vision', data.vision);
      addField('Brand Philosophy', data.philosophy);
      
      addDivider();
      addTitle('03. Operational DNA');
      addField('Problem Solving', data.problemSolving);
      addField('Products & Services', data.productsServices);
      addField('Delivery Model', data.deliveryModelOther || data.deliveryModel.join(', '));

      pdf.addPage();
      addHeaderFooter();
      y = 80;

      addTitle('04. Market Positioning');
      addField('Ideal Customers', data.idealCustomers);
      addField('Key Benefits', data.customerBenefitsOther || data.customerBenefits.join(', '));
      addField('Differentiation', data.differentiation);
      addField('Competitors', data.competitors);
      
      addDivider();
      addTitle('05. Brand Aura');
      addField('Personality', data.personality);
      addField('Tone of Voice', data.tone);
      addField('Brand Feel', data.brandFeel.join(', '));
      addField('Emotional Outcome', data.customerEmotionalOutcome.join(', '));

      addDivider();
      addTitle('06. Visual & Future');
      addField('Color & Symbolism', data.colorSymbols);
      addField('Future Plans', data.futurePlans);

      // Save
      const brandFileName = (data.name || 'Brand').replace(/[^a-z0-9]/gi, '_');
      pdf.save(`${brandFileName}_Discovery.pdf`);
  ...
  useEffect(() => {
    const handleSaveEvent = () => {
      console.log('Discovery: Received Global Save Event');
      handleManualSave();
    };
    const handleExportEvent = () => {
      console.log('Discovery: Received Global Export Event');
      handleExportPDF();
    };
    
    window.addEventListener('brandforge:save-discovery', handleSaveEvent);
    window.addEventListener('brandforge:export-discovery', handleExportEvent);
    
    return () => {
      window.removeEventListener('brandforge:save-discovery', handleSaveEvent);
      window.removeEventListener('brandforge:export-discovery', handleExportEvent);
    };
  ...
  const renderIntro = () => (
    <div className="text-center space-y-8 py-10">
      <div className="w-20 h-20 bg-brand-50 rounded-full flex items-center justify-center mx-auto text-brand-600 shadow-inner">
        <Rocket className="w-10 h-10" />
      </div>
      <div className="space-y-4">
        <h2 className="text-4xl font-bold text-brand-900 tracking-tight font-display">Let’s Build Your Brand 🚀</h2>
        <p className="body-lg text-brand-500 max-w-xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-6 text-sm text-brand-400 font-medium">
    ...
    <Card title="Client Details" icon={User} className="w-full">
      <div className="space-y-[var(--space-gap)]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-[var(--space-gap)]">
          ...
          <div>
            <label className="text-sm font-bold text-brand-900 mb-1 block">Phone Number</label>
            <p className="caption mb-2">👉 Quick reach number?</p>
            <Input 
              type="tel"
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Can we contact you?</label>
          <p className="caption mb-2">👉 Are you open to us reaching out via WhatsApp or SMS if needed?</p>
              ...
              <label key={opt} className="flex items-center gap-3 cursor-pointer group">
                <div className={cn(
                  ...
                  (opt === 'Yes' ? data.canContact : !data.canContact)
                    ...
                    : "border-brand-200 group-hover:border-brand-400"
                )}>
                  {(opt === 'Yes' ? data.canContact : !data.canContact) && (
                ...
                />
                <span className="text-sm font-medium text-brand-700">{opt}</span>
              </label>
            ))}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {STAGES.map(stage => (
                <button
                  ...
                  className={cn(
                    "p-3 sm:p-2 rounded-[var(--radius-card)] border text-xs font-medium transition-all text-left",
                    data.stage === stage
                      ? "bg-brand-600 border-brand-600 text-white cursor-pointer"
                      : "bg-white border-brand-200 text-brand-600 hover:border-brand-400 cursor-pointer"
                  )}
                >
                  {stage}
                </button>
  ...
  const renderSection4 = () => (
    <Card title="Your Story" icon={BookOpen} className="w-full">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What is the mission of this Brand?</label>
          <p className="caption mb-2">👉 Why does your business exist? What are you trying to achieve?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What is the vision of this Brand?</label>
          <p className="caption mb-2">👉 Where do you see your business in the future?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What is Your Brand Philosophy</label>
          <p className="caption mb-2">👉 What core idea or belief drives how you do business?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What Problem is your Business Solving?</label>
          <p className="caption mb-2">👉 What gap did you notice that made you start this business?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What service or products do you offer?</label>
          <p className="caption mb-2">👉 What exactly do you provide to solve this problem?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Who are you offering the Services to?</label>
          <p className="caption mb-2">👉 Who are your ideal customers?</p>
          <Textarea 
  ...
  const renderSection5 = () => (
    <Card title="How Your Business Works" icon={Zap} className="w-full">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">How are you offering this service?</label>
          <p className="caption mb-2">👉 How do customers access or buy from you?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {DELIVERY_MODELS.map(model => (
              <button
                ...
                className={cn(
                  "p-2 rounded-[var(--radius-card)] border text-xs font-medium transition-all text-left flex items-center gap-2",
                  data.deliveryModel.includes(model)
                    ? "bg-brand-600 border-brand-600 text-white cursor-pointer"
                    : "bg-white border-brand-200 text-brand-600 hover:border-brand-400 cursor-pointer"
                )}
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.deliveryModel.includes(model) ? "bg-white border-white" : "border-brand-300")}>
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What benefits do you provide for your customers?</label>
          <p className="caption mb-2">👉 What do customers gain from choosing you?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {BENEFITS.map(benefit => (
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.customerBenefits.includes(benefit) ? "bg-white border-white" : "border-brand-300")}>
  ...
  const renderSection6 = () => (
    <Card title="Your Brand Foundation" icon={Shield} className="w-full">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What are your Brand Core Values?</label>
          <p className="caption mb-2">👉 What principles guide how your business operates?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {VALUES.map(val => (
              <button
                key={val}
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.coreValues.includes(val) ? "bg-white border-white" : "border-brand-300")}>
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What are your Brand Strengths?</label>
          <p className="caption mb-2">👉 What do you do really well?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {STRENGTHS.map(str => (
              <button
                key={str}
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.strengths.includes(str) ? "bg-white border-white" : "border-brand-300")}>
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What are your Weakness / Limitations?</label>
          <p className="caption mb-2">👉 Where do you feel your business could improve?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {WEAKNESSES.map(weak => (
              <button
                key={weak}
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.weaknesses.includes(weak) ? "bg-white border-white" : "border-brand-300")}>
    ...
    <Card title="Brand Identity" icon={Palette} className="w-full">
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="text-sm font-bold text-brand-900 mb-1 block">What is your Brand Name?</label>
            <p className="caption mb-2">👉 Logo appearance name?</p>
            <Input 
          ...
          <div>
            <label className="text-sm font-bold text-brand-900 mb-1 block">Colour & Symbols</label>
            <p className="caption mb-2">👉 Preferred colors or symbols?</p>
            <Input 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What is the meaning or story behind your Brand Name?</label>
          <p className="caption mb-2">👉 Tell us the story or inspiration behind your name</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Brand Feel / Visual Direction (Select up to 4)</label>
          <p className="caption mb-2">👉 How should your brand look and feel?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {BRAND_FEELS.map(feel => (
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.brandFeel.includes(feel) ? "bg-white border-white" : "border-brand-300")}>
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Emotional Experience (Select up to 4)</label>
          <p className="caption mb-2">👉 How do you want your customers to feel?</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {EMOTIONAL_OUTCOMES.map(outcome => (
              ...
              >
                <div className={cn("w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0", data.customerEmotionalOutcome.includes(outcome) ? "bg-white border-white" : "border-brand-300")}>
  ...
  const renderSection8 = () => (
    <Card title="Competition" icon={Trophy} className="w-full">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Who are your competitors and why?</label>
          <p className="caption mb-2">👉 List 2–3 competitors and why they are similar</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What are their Strengths and weaknesses?</label>
          <p className="caption mb-2">👉 What do they do well—and where do they fall short?</p>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What makes you different from your competitors?</label>
          <p className="caption mb-2">👉 Why should people choose you instead?</p>
          <Textarea 
  ...
  const renderSection9 = () => (
    <Card title="Project Details" icon={Clock} className="w-full">
      <div className="space-y-6">
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">What is your deadline for this project?</label>
          <p className="caption mb-2">👉 When would you like this completed?</p>
          <Input 
            type="date"
            value={data.deadline}
        ...
        <div>
          <label className="text-sm font-bold text-brand-900 mb-1 block">Future Plans / Innovation</label>
          <p className="caption mb-2">👉 Tell us anything coming soon we should consider</p>
          <Textarea 
        ...
        <div className="space-y-8">
          ...
          <div className="flex justify-center gap-3">
            <button
              ...
              className={cn(
                "flex flex-col items-center gap-2 px-4 py-4 rounded-[var(--radius-section)] border transition-all font-medium w-36 text-center",
                importMethod === 'manual' 
                      ...
                      {forms.length === 0 ? (
                        ...
                        forms.map(f => (
                          <button 
                            ...
                            className="w-full text-left p-3 hover:bg-brand-50 rounded-[var(--radius-card)] border border-brand-100 text-sm truncate transition-colors flex items-center justify-between group"
                          >
                            <span className="font-medium text-brand-700">{f.name}</span>
                            <ChevronRight className="w-4 h-4 text-brand-300 group-hover:text-brand-500 transition-colors" />
                          </button>
                        ))
                      )}
                    </div>
                      ...
                      {formResponses.length === 0 ? (
                        ...
                        formResponses.map(r => (
                          <button 
                            ...
                            className="w-full text-left p-3 hover:bg-brand-50 rounded-[var(--radius-card)] border border-brand-100 text-xs truncate transition-colors flex items-center justify-between group"
                          >
                            <div className="flex flex-col">
                              <span className="font-bold text-brand-700">{r.respondentEmail || 'Anonymous'}</span>
                              <span className="text-[10px] text-brand-400">Submitted {new Date(r.lastSubmittedTime).toLocaleString()}</span>
                            </div>
                            <ChevronRight className="w-4 h-4 text-brand-300 group-hover:text-brand-500 transition-colors" />
                          </button>
                        ))
                  ...
                  <React.Fragment key={idx}>
                    ...
                    >
                      ...
                      <p className={cn(
                        ...
                        isCurrent ? "text-brand-600" : "text-brand-400"
                      )}>{phase.label}</p>
                    </div>
                    {!isLast && (
                      <div className="flex-1 min-w-[4px] md:min-w-[6px] lg:min-w-[12px] h-0.5 bg-brand-100 mx-0.5 lg:mx-1 relative">
                        {isPast && (
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: '100%' }}
      ...
      {showCompleteModal && (
        <div className="fixed inset-0 bg-brand-950/40 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <motion.div 
            ...
            className="bg-white rounded-[var(--radius-section)] p-8 max-w-md w-full shadow-2xl border border-brand-100 text-center space-y-6 relative"
          >
            <button 
              onClick={() => setShowCompleteModal(false)}
              className="absolute top-6 right-6 p-2 hover:bg-brand-50 rounded-xl text-brand-400 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>

            <div className="w-20 h-20 bg-brand-50 text-brand-600 rounded-[var(--radius-card)] flex items-center justify-center mx-auto shadow-inner">
              <CheckCircle2 className="w-10 h-10" />
            </div>
            <div className="space-y-2">
              <h3 className="text-2xl font-bold text-brand-900 font-display">Discovery Complete!</h3>
              <p className="body text-brand-500">Your brand intelligence has been captured. What would you like to do next?</p>
            </div>
            <div className="flex flex-col items-center gap-4 pt-4">
              <Button 
                ...
                size="md"
                className="w-full py-6 gap-3 label border-brand-200 group"
              >
                <Download className="w-4 h-4 group-hover:translate-y-0.5 transition-transform" />
                <span className="tracking-widest">Download Discovery PDF</span>
              </Button>

              <Button 
                onClick={() => handleFinalComplete('strategy')}
                disabled={isCompleting || isExporting}
                size="md"
                className="w-full py-6 gap-3 label bg-brand-600 hover:bg-brand-700 shadow-xl shadow-brand-100"
              >
                {isCompleting ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                  <>
                    <BookOpen className="w-5 h-5" />
                    <span className="tracking-widest">Go to Strategy Engine</span>
                  </>
                )}
              </Button>
              ...
              <button 
                ...
                className="mt-2 text-brand-400 hover:text-brand-600 font-bold text-xs uppercase tracking-widest transition-colors label"
              >
                Back to Dashboard
              </button>
            </div>
